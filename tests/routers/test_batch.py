...
class TestBatchPatients:
    ...
    def test_batch_create_patients_success(self, client, auth_headers_doctor, test_tenant):
        ...
        response = client.post("/api/v1/batch/patients", json=patients_data, headers=headers)
    ...
    def test_batch_create_patients_atomic_rollback(self, client, auth_headers_doctor, db):
        ...
        patients = db_session.query(Patient).filter(Patient.email == "valid@example.com").all()
    ...
    def test_batch_update_patients_success(self, client, auth_headers_doctor, test_patient, db):
        ...
        updates = [
            {"id": str(test_patient.id), "phone": "+9999999999"},
            {"id": str(patient2.id), "email": "newemail@example.com"},
        ]

        response = client.put("/api/v1/batch/patients", json=updates, headers=headers)
    ...
    def test_batch_update_nonexistent_patient(self, client, auth_headers_doctor):
        ...
        updates = [{"id": "00000000-0000-0000-0000-000000000000", "phone": "+9999999999"}]

        response = client.put("/api/v1/batch/patients", json=updates, headers=auth_headers_doctor)
    ...
    def test_batch_delete_patients_success(self, client, auth_headers_doctor, test_patient, db):
        ...
        response = client.delete(
            "/api/v1/batch/patients", params={"patient_ids": patient_ids}, headers=headers
        )
        ...
        patients = (
            db_session.query(Patient).filter(Patient.id.in_([test_patient.id, patient2.id])).all()
        )
        assert len(patients) == 0
    ...
    def test_batch_delete_partial_success(self, client, auth_headers_doctor, test_patient):
        ...
        patient_ids = [str(test_patient.id), "00000000-0000-0000-0000-000000000000"]  # Non-existent
    ...
    def test_batch_operations_require_auth(self, unauthenticated_client):
        ...
        response = unauthenticated_client.delete(
            "/api/v1/batch/patients", params={"patient_ids": []}
        )
        assert response.status_code == status.HTTP_401_UNAUTHORIZED
    ...
    def test_batch_operations_tenant_isolation(self, client, auth_headers_doctor, test_patient, db):
        ...
        updates = [{"id": str(other_patient.id), "phone": "+1111111111"}]
    ...
    def test_batch_create_empty_list(self, client, auth_headers_doctor):
        ...
        response = client.post("/api/v1/batch/patients", json=[], headers=auth_headers_doctor)
