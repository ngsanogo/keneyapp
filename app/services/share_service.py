...
from app.schemas.medical_record_share import ShareCreate, ShareResponse, SharedMedicalRecord
...
def create_share(
    ...
    patient = db.query(Patient).filter(
        Patient.id == share_data.patient_id,
...
def get_share_by_token(
    ...
    share = db.query(MedicalRecordShare).filter(
        MedicalRecordShare.share_token == token
    ).first()
...
def get_shared_medical_record(
    ...
    if share.scope == ShareScope.FULL_RECORD:
        ...
        record.documents = [
            ...
            for doc in patient.documents if not doc.deleted_at
        ]
