...
class ProSanteConnectService:
    ...
    def get_authorization_url(self, state: str) -> str:
        ...
        if not self.client_id:
            raise ValidationError(
                "Pro Santé Connect not configured. Set PSC_CLIENT_ID."
            )
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code != 200:
                    raise AuthenticationError(
                        f"PSC token exchange failed: {response.text}"
                    )
    ...
    async def authenticate_with_psc(
        ...
        cps_record = (
            self.db.query(HealthcareProfessionalCPS)
            .filter(
    ...
    def get_cps_by_user_id(self, user_id: UUID) -> Optional[HealthcareProfessionalCPS]:
        ...
        return (
            self.db.query(HealthcareProfessionalCPS)
            .filter(HealthcareProfessionalCPS.user_id == user_id)
            .first()
        )
