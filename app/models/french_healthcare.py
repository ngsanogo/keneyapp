...
class PatientINS(Base):
    ...
    # Patient reference
    patient_id = Column(PGUUID(as_uuid=True), ForeignKey("patients.id", ondelete="CASCADE"), nullable=False, unique=True, index=True)
    
    # INS Components (Format: 1 YY MM RR DDD KKK CC)
    # Example: 1 84 12 75 123 456 78
    ins_number = Column(String(15), nullable=False, unique=True, index=True)  # Full INS (13 digits)
    ...
    verification_method = Column(String(50), nullable=True)  # teleservice_ins, carte_vitale, etc.
    verification_operator_id = Column(PGUUID(as_uuid=True), ForeignKey("users.id"), nullable=True)
    ...
    verification_operator = relationship("User", foreign_keys=[verification_operator_id])
...
class HealthcareProfessionalCPS(Base):
    ...
    cps_number = Column(String(20), nullable=False, unique=True, index=True)  # Numéro CPS (8 digits)
    rpps_number = Column(String(11), nullable=True, index=True)  # Répertoire Partagé des Professionnels de Santé
    adeli_number = Column(String(9), nullable=True, index=True)  # ADELI (old identification)
    
    # Professional details (from RPPS)
    profession_code = Column(String(10), nullable=True)  # Code profession (ex: 10 = Médecin)
    ...
    psc_sub = Column(String(100), nullable=True, unique=True)  # Subject identifier from Pro Santé Connect
...
class DMPIntegration(Base):
    ...
    # DMP details
    dmp_id = Column(String(50), nullable=True, unique=True, index=True)  # INS-based DMP identifier
    dmp_consent = Column(Boolean, nullable=False, default=False)
    dmp_consent_date = Column(DateTime(timezone=True), nullable=True)
    dmp_access_mode = Column(String(20), nullable=True)  # normal, urgence, bris_de_glace
...
class MSSanteMessage(Base):
    ...
    # Message references
    internal_message_id = Column(PGUUID(as_uuid=True), ForeignKey("messages.id"), nullable=True, index=True)
    ...
    # Status
    status = Column(String(20), nullable=False, default="pending")  # pending, sent, delivered, read, failed
