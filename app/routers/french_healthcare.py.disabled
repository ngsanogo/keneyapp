...
async def verify_patient_ins(
    ...
    **Rate Limit**: 10 requests per minute
    ...
    try:
        ...
        # Clear patient cache
        cache_clear_pattern(
            f"patients:detail:{current_user.tenant_id}:{data.patient_id}"
        )
        cache_clear_pattern(f"patients:list:{current_user.tenant_id}:*")

        # Metrics
        patient_operations_total.labels(
            action="ins_verification", tenant_id=str(current_user.tenant_id)
        ).inc()

        message = (
            "INS verified successfully"
            if verification_success
            else "INS verification pending or failed"
        )
        ...
        return INSVerificationResponse(
            ...
            verified_at=(
                ins_record.verified_at.isoformat() if ins_record.verified_at else None
            ),
            expires_at=(
                ins_record.expires_at.isoformat() if ins_record.expires_at else None
            ),
            message=message,
            identity_traits=(
                {
                    ...
                    "birth_date": (
                        ins_record.birth_date.isoformat()
                        if ins_record.birth_date
                        else None
                    ),
                    "birth_location": ins_record.birth_location,
                    "gender_code": ins_record.gender_code,
                }
                if ins_record.birth_name
                else None
            ),
        )
        ...
        identity_traits=(
            {
                ...
                "first_names": ins_record.first_names,
                "birth_date": (
                    ins_record.birth_date.isoformat() if ins_record.birth_date else None
                ),
                "birth_location": ins_record.birth_location,
                "gender_code": ins_record.gender_code,
            }
...
@limiter.limit("10/minute")
async def get_psc_authorization_url(request: Request, db: Session = Depends(get_db)):
    """
...
async def psc_callback(
    request: Request, data: PSCCallbackRequest, db: Session = Depends(get_db)
):
    """
    ...
    **Note**: This endpoint is public (completes authentication flow)
    ...
    try:
        user, cps_record, access_token = await psc_service.authenticate_with_psc(
            code=data.code, tenant_id=default_tenant.id
        )
...
async def get_my_cps_details(
    ...
    **Rate Limit**: 30 requests per minute
    ...
    return {
        ...
        "expiry_date": (
            cps_record.expiry_date.isoformat() if cps_record.expiry_date else None
        ),
        "psc_last_login": (
            cps_record.psc_last_login.isoformat() if cps_record.psc_last_login else None
        ),
    }
