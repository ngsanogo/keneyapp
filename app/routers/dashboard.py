...
def get_dashboard_stats(db: Session = Depends(get_db)):
    ...
    recent_patients = db.query(func.count(Patient.id)).scalar()  # Assuming we'd have a created_at field
    
    recent_appointments = db.query(func.count(Appointment.id)).filter(
        Appointment.appointment_date >= week_ago
    ).scalar()
    
    recent_prescriptions = db.query(func.count(Prescription.id)).filter(
